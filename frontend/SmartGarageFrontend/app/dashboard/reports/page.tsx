...
export default function InvoicePage() {
    ...
    const serviceList = currentVisit?.pack?.services?.length
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen flex flex-col lg:flex-row gap-6">
            ...
            <Card
                ref={contentRef}
                className="w-full lg:max-w-3xl p-6 border border-gray-300 shadow-lg bg-white rounded-lg"
            >
                {/* Header */}
                    ...
                    <div>
                        ...
                        <CardDescription>
                            <Typography size="sm" as="p" className="text-gray-500">
                                {currentVisit ? formatDateWithOrdinal(currentVisit.visitDate) : "N/A"}
                            </Typography>
                        </CardDescription>
                    </div>
                </CardHeader>
                ...
                <CardContent className="flex flex-col justify-between h-full space-y-6 sm:space-y-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                        <div>
                            ...
                            <Typography weight="bold">{currentVehicle?.client?.name ?? "N/A"}</Typography>
                                ...
                                <TableBody>
                                    {selectedVehicle && currentVehicle ? (
                                        <TableRow key={currentVehicle.vehiclePlate}>
                                            <TableCell className="px-4 py-2">{currentVehicle.vehiclePlate}</TableCell>
                    ...
                    <div>
                        ...
                        <Typography>{currentVisit?.pack?.packName || "CUSTOM PACK"}</Typography>
                    </div>
                            ...
                            <Table className="min-w-[600px] w-full border border-gray-200 rounded-md overflow-hidden">
                                ...
                                <TableBody>
                                    {serviceList.length ? (
                                        serviceList.map((service, i) => (
                                            <TableRow key={i}>
                                                <TableCell className="text-left px-4 py-2">{service.serviceName || "Service"}</TableCell>
                                                <TableCell className="text-right px-4 py-2">1</TableCell>
                                                <TableCell className="text-right px-4 py-2">${service.price.toFixed(2)}</TableCell>
                                                <TableCell className="text-right px-4 py-2">${service.price.toFixed(2)}</TableCell>
                                            </TableRow>
                                        ))
                                    ) : (
                                ...
                                <TableFooter>
                                    <TableRow className="bg-gray-50 font-semibold border-t">
                                        <TableCell colSpan={3} className="px-4 py-2 text-left">Total</TableCell>
                                        <TableCell className="px-4 py-2 text-right">
                                            ${serviceList.reduce((sum, s) => sum + s.price, 0).toFixed(2)}
                                        </TableCell>
                                    </TableRow>
                                </TableFooter>
                            </Table>
            ...
            <div className="w-full max-w-md flex flex-col gap-6">
                <Card className="p-6 shadow-xl rounded-2xl bg-gradient-to-br from-white to-gray-100 border border-gray-200">
                    <div className="text-center mb-4">
                        <Typography size="lg" weight="bold" className="text-gray-700">Invoice Options</Typography>
                        <Typography size="sm" className="text-gray-500">Select vehicle & visit</Typography>
                    </div>
                    ...
                    <Button
                        ...
                        className="w-full flex items-center justify-center gap-2 mt-4 bg-blue-600 hover:bg-blue-700 text-white shadow-lg rounded-lg"
                    >
                    ...
                    <Button
                        ...
                        disabled={!selectedVehicle || !selectedVisit}
                        className="w-full flex items-center justify-center gap-2 bg-gray-50 hover:bg-gray-100 shadow rounded-lg border border-gray-200"
                    >
